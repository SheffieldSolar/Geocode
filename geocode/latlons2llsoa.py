...
def parse_options():
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Specify to use Data Zones in Scotland.",
    )
    ...
    if options.outfile.is_file():
        check = query_yes_no(
            f"The outfile '{options.outfile}' already exists, are you sure you "
            "wish to overwrite?",
            "no",
        )
        if not check:
...
def main():
    ...
    with Geocoder() as geo:
        df["llsoacd"] = geo.reverse_geocode_llsoa(
            df[["latitude", "longitude"]].to_numpy(), options.datazones
        )
