...
def geocode(
    ...
    if postcodes is None and addresses is None:
        raise utilities.GenericException(
            "You must pass either postcodes or addresses, or both."
        )
    ...
    logging.debug(
        "Geocoding %s postcodes (%s addresses)", len(postcodes), len(addresses)
    )
    ...
    results = inputs.merge(
        self.cache, how="left", on=["input_postcode", "input_address"]
    )
    ...
    if use_cpo.any():
        ...
        results.loc[use_cpo, results_cols] = cpo_manager.cpo_geocode(
            inputs.loc[use_cpo, ["input_postcode"]]
        )[results_cols]
        logging.debug(
            "Successfully geocoded %s postcodes using Code Point Open",
            (results.latitude.notnull()).sum(),
        )
    use_gmaps = results.latitude.isnull()
    if use_gmaps.any():
        logging.debug(
            "Geocoding %s postcodes/addresses using Google Maps", use_gmaps.sum()
        )
        gmaps_geocode = lambda i: gmaps_manager.gmaps_geocode_one(
            i.input_postcode, i.input_address
        )
        results.loc[use_gmaps, results_cols] = inputs.loc[use_gmaps].apply(
            gmaps_geocode, axis=1
        )[results_cols]
        logging.debug(
            "Successfully geocoded %s postcodes/addresses using Google Maps",
    ...
    self.cache = cache.drop_duplicates(
        subset=["input_postcode", "input_address"], keep="last", ignore_index=True
    )
    return results[results_cols].to_records(index=False)


def geocode_one(
    self, postcode: Optional[str] = None, address: Optional[str] = None
    ...
    if postcode is None and address is None:
        raise utilities.GenericException(
            "You must specify either a postcode or an address, or both."
        )
    if address is None:
...
def reverse_geocode(
    ...
    if "GSPGroup" in joined.columns:
        joined["GSPGroup"] = joined["GSPGroup"].where(
            pd.notna(joined["GSPGroup"]), None
        )
...
def _fetch_from_ons_api(url, proxies=None, ssl_verify=True):
    ...
    while exceeded_transfer_limit:
        ...
        success, api_response = fetch_from_api(
            url_, proxies=proxies, ssl_verify=ssl_verify
        )
        if success:
            page = json.loads(api_response.text)
            exceeded_transfer_limit = (
                "properties" in page
                and "exceededTransferLimit" in page["properties"]
                and page["properties"]["exceededTransferLimit"]
            )
            pages.append(page)
            offset += record_count
        else:
            raise GenericException(
                "Encountered an error while extracting LLSOA data from ONS API."
            )
    return pages
