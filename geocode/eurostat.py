...
class Eurostat:
    ...
    def load_nuts_boundaries(
        ...
        if nuts_boundaries_cache_contents is not None:
            logging.debug(
                "Loading %s NUTS%s boundaries from cache ('%s')",
                year,
                level,
                cache_label,
            )
        ...
        logging.info(
            "Extracting the %s NUTS%s boundary data from Eurostat (this only needs to be "
            "done once)",
            year,
            level,
        )
