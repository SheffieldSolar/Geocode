...
class NationalGrid:
    ...
    def _load_gsp_lookup_20181031(self):
        ...
        gsp_lookup_cache_contents = self.cache_manager.retrieve(
            self.gsp_lookup_20181031_cache_file
        )
        if gsp_lookup_cache_contents is not None:
            logging.debug(
                "Loading GSP lookup data from cache ('%s')",
                self.gsp_lookup_20181031_cache_file,
            )
        ...
        if success:
            ...
            raise utils.GenericException(
                "Encountered an error while extracting GSP lookup data from ESO " "API."
            )
        self.cache_manager.write(self.gsp_lookup_20181031_cache_file, gsp_lookup)
        logging.info(
            "GSP lookup extracted and pickled to '%s'",
            self.gsp_lookup_20181031_cache_file,
        )
        return gsp_lookup
            ...
            with ZipFile(zip_file, "r") as zip_ref:
                if target_file in zip_ref.namelist():
                    with zip_ref.open(target_file) as file:
                        ...
                        gsp_regions = gpd.GeoDataFrame.from_features(
                            raw["features"], crs=raw["crs"]["properties"]["name"]
                        )
    ...
    def _load_gsp_boundaries_20220314(self):
        ...
        if gsp_boundaries_cache_contents is not None:
            logging.debug(
                "Loading 20220314 GSP boundaries from cache ('%s')",
                self.gsp_boundaries_20220314_cache_file,
            )
            return gsp_boundaries_cache_contents
        logging.info(
    ...
    def _load_dno_boundaries(self):
        ...
        ESO Data Portal API.
        """
        dno_boundaries_cache_contents = self.cache_manager.retrieve(
            self.dno_boundaries_cache_file
        )
        if dno_boundaries_cache_contents is not None:
            logging.debug(
                "Loading DNO boundaries from cache ('%s')",
                self.dno_boundaries_cache_file,
            )
        ...
        if success:
            ...
            for f in raw["features"]:
                ...
                dno_names[region_id] = (
                    f["properties"]["Name"],
                    f["properties"]["LongName"],
                )
        else:
            raise utils.GenericException(
                "Encountered an error while extracting DNO License Area "
                "boundary data from ESO API."
            )
        dno_regions = {
            region_id: (dno_regions[region_id], dno_regions[region_id].bounds)
            for region_id in dno_regions
        }
        self.cache_manager.write(
            self.dno_boundaries_cache_file, (dno_regions, dno_names)
        )
        logging.info(
            "DNO License Area boundaries extracted and pickled to '%s'",
            self.dno_boundaries_cache_file,
        )
    ...
    def reverse_geocode_gsp_20181031(
